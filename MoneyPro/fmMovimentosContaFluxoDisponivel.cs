...
namespace MoneyPro
{
    public partial class fmMovimentosContaFluxoDisponivel : MoneyPro.Base.fmBaseTopoRodape
    {
        const int LarguraMinima = 70;
        private CelulaSelecionada Selecao = new CelulaSelecionada();

        private Tipo.TipoFluxoCaixa TipoFluxo { get; set; }
        ...
        private DateTime DataBase
        {
            ...
            set
            {
                if (_DataBase.Year != value.Year || _DataBase.Month != value.Month)
                {
                    ...
                    switch (this.TipoFluxo)
                    {
                        case TipoFluxoCaixa.Disponivel:
                            this.labelTopo.Text = "Contas Disponíveis em " + _DataBase.ToString("MM/yyyy");
                            break;
                        case TipoFluxoCaixa.Credito:
                            this.labelTopo.Text = "Contas Crédito em " + _DataBase.ToString("MM/yyyy");
                            break;
                        default:
                            break;
                    }
        ...
        private static fmMovimentosContaFluxoDisponivel _singleton;

        private fmMovimentosContaFluxoDisponivel(Form origem, int usuarioID, Tipo.TipoFluxoCaixa tipo)
        {
            InitializeComponent();
            this.Origem = origem;
        ...
        public static fmMovimentosContaFluxoDisponivel CreateInstance(Form origem, int usuarioID, Tipo.TipoFluxoCaixa tipo)
        {
            if (_singleton == null)
            {
                _singleton = new fmMovimentosContaFluxoDisponivel(origem, usuarioID, tipo);
            }
            return _singleton;
        }
        ...
        private void CarregarDados(int usuarioID)
        {
            ...
            fluxoCaixaDataGridView.DataSource = bll.FluxoCaixa(TipoFluxo, usuarioID, DataBase);
        ...
        private void FluxoCaixaDataGridView_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (e.Value != null)
            {
        ...
        private void FluxoCaixaDataGridView_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
        ...
        private void fmMovimentosContaFluxoDisponivel_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void FluxoCaixaDataGridView_DoubleClick(object sender, EventArgs e)
        {
        ...
        private void FluxoCaixaDataGridView_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void AnalisarCelulaSelecionada()
        {
            if (fluxoCaixaDataGridView.CurrentCell != null)
            {
                ...
                int contaID = (int)fluxoCaixaDataGridView.CurrentRow.Cells["fluxo_ContaID"].Value;

                if ((short)fluxoCaixaDataGridView.CurrentRow.Cells["fluxo_TipoLinha"].Value == 2 && categoriaID != 0)
                {
                    // Exibe informações da categoria selecionada
                ...
                else if ((short)fluxoCaixaDataGridView.CurrentRow.Cells["fluxo_TipoLinha"].Value == 2 && contaID != 0)
                {
                    // Exibe informações da conta selecionada
                    ...
                    if (nome.Contains("fluxo_Dia"))
                    {
                        ...
                        // Exibe os detalhes no grid inferior
                        ExibirTransferenciasContaNoDia(UsuarioID, contaID, diaSelecionado);
                    ...
                    else
                    {
                        ...
                        // Exibe os detalhes no grid inferior
                        ExibirTransferenciasCategoriaNoMes(UsuarioID, contaID, mesSelecionado);
        ...
        {
            PesquisaBLL bll = new PesquisaBLL();
            detalhesMovimentacaoBindingSource.DataSource = bll.ListarMovimentosCategoriaNoDia(TipoFluxo, usuarioID, categoriaID, diaSelecionado);
        }

        private void ExibirTransferenciasContaNoDia(int usuarioID, int contaID, DateTime diaSelecionado)
        {
            PesquisaBLL bll = new PesquisaBLL();
            detalhesMovimentacaoBindingSource.DataSource = bll.ListarTransferenciasContaNoDia(usuarioID, contaID, diaSelecionado);
        }
        ...
        {
            PesquisaBLL bll = new PesquisaBLL();
            detalhesMovimentacaoBindingSource.DataSource = bll.ListarMovimentosCategoriaNoMes(TipoFluxo, usuarioID, categoriaID, mesSelecionado);
        }

        private void ExibirTransferenciasCategoriaNoMes(int usuarioID, int contaID, DateTime mesSelecionado)
        {
            PesquisaBLL bll = new PesquisaBLL();
            detalhesMovimentacaoBindingSource.DataSource = bll.ListarTransferenciasContaNoMes(usuarioID, contaID, mesSelecionado);
        }
